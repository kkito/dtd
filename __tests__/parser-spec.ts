...
test('.matchComments' , () => {
    ...
    const result = Parser.matchComments(`${firstComment} <!sswef > ${secondComment}`)
...
test('.matchDTDTags' , () => {
    ...
    const secondTag = "<!ATTLIST smilCustomTest \
    id    ID    #REQUIRED \
    ...
    >"
    const tags = Parser.matchDTDTags(` place all customTest elements used in the SMIL files, for presentation to the user. ${firstTag} ${secondTag}`)
